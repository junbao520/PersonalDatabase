...
| Code | Style |
|------|-------|
|Source Files | One Namespace per file. |
...
|Source File | Pascal Case. Always match Class name and file name. Avoid including more than one Enum (global), or Delegate (global) per file. Use a descriptive file name when containing multiple Class, Enum, or Delegates. Example: ``` MyClass.cs => public class MyClass  {…} ```|
...
2. Avoid putting multiple irrelevant classes in a single file.
